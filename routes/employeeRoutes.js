...
router.get('/get', authenticateToken, (req, res, next) => getEmployee(req, res, next, { populate: ['department', 'manager'] }));

// router.get('/get', [authenticateToken,checkRole('admin','manager')], getEmployee);
...
router.delete("/delete/:id", [authenticateToken, checkRole('admin', 'manager')], deleteEmployee);

// Update employee (only admin can update)
router.put("/update/:id", [authenticateToken], upload.single('profileImage'), updateEmployee);
